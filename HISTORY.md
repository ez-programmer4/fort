...
**Follow-up (same day):** user wanted the combo to sit behind an explicit "+ Add product" button rather than always being visible. Added an `addingProduct` toggle: the section now shows a dashed "+ Add product" button by default; clicking it swaps in the search-combo (auto-focused so typing can start immediately) plus a small "×" button to collapse back to the button. The combo stays open after each add so multiple items can still be added in a row without re-clicking — only the *first* interaction requires the button. Also added an `autoFocus` prop to the shared `Combobox` component (`frontend/src/components/ui/combobox.tsx`) to support this — focuses (and opens) the input on mount, reusable by any other button-revealed combo. Switching location now also collapses the add-product UI, consistent with it already clearing the cart. `tsc --noEmit` clean, full 19-page sweep clean.
