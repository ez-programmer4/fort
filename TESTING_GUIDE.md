...
| #        | Scenario                     | Steps                                                                                                      | Expected                                                                                                                  |
| -------- | ---------------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| 10.1 ✅  | Dispense — single item       | Sales → New Dispense, pick location, use the "Add product" search-combo (type a name/code/batch), select a result | Item added to the cart instantly; quantity/price editable in Step 2, submit creates the order (`DSP-00001`), stock decreases at that location/batch |
| 10.2 ✅  | Dispense — multiple items    | Add several items via the search-combo before confirming                                                   | All lines processed atomically; one order, multiple `DispenseItem`s                                                       |
| 10.1a ✅ | Add-product combo — search   | Type part of a product name, code, or batch number into "Add product"                                      | Dropdown narrows to matching batches (debounced ~300ms); each option shows code, batch, quantity available, price, and expiry status (e.g. "45d left"/"EXPIRED") in the sublabel |
...
| 10.1c ✅ | Add-product combo — already in cart | Add a batch, then reopen the combo                                                                  | That batch no longer appears in the dropdown (can't be added twice); still visible/removable in the "Selected items" list |
...
| 13.2 ✅ | Sales summary                   | Check today's total/count and last-7-days total/count                             | Matches what Sales History shows for the same window                                                        |
| 13.3 ✅ | Alert counts                    | Quick-link card                                                                   | Counts match the Alerts page exactly                                                                        |
| 13.4 ✅ | Top moving products             | Check the ranked list                                                             | Matches actual dispense quantities over the last 30 days                                                    |
| 13.5 ✅ | Recent sales table              | Check the list                                                                    | Matches the most recent Sales History entries                                                               |
| 13.6 ✅ | Period switch (7d/30d/90d/etc.) | Change the period selector on the analytics section                               | Profit/sales trend charts, top customers, and top products by margin/volume all re-fetch for the new window |
