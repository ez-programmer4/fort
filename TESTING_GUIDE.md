...
| 13.2 ✅  | Sales summary                   | Check today's total and Monthly Sales (current calendar month) total/count        | Matches what Sales History shows for today and month-to-date (1st of this month through now, NOT a rolling 30 days) |
| 13.3 ✅  | Alert counts                    | Quick-link cards (Low Stock, Expiring)                                            | Counts match the Alerts page exactly                                                                         |
| 13.4 ✅  | Fast Movers table                | Check the "Fast Movers" table                                                     | Ranked by quantity sold in the last 30 days (fixed window); each row shows qty sold, revenue, and gross profit |
| 13.5 ✅  | Recent sales table              | Check the list                                                                    | Matches the most recent Sales History entries                                                                |
| 13.6 ✅  | Period switch (7d/30d/90d/etc.) | Change the period selector on the Performance Overview section                    | Profit KPIs, profit/sales-vs-purchases trend charts, top customers, payment mix, and the Top Products table all re-fetch for the new window (Sales Overview and Fast/Slow Movers are unaffected — they have their own independent windows) |
...
| 13.11 ✅ | Total Buyers KPI                | Compare against distinct customers with at least one order created this calendar month | Matches; a customer who last bought before the 1st of this month is NOT counted, even if they've bought before |
| 13.12 ✅ | Sales Overview — granularity toggle | Switch the Weekly / Monthly / Yearly buttons above the Sales Overview charts   | Revenue and Order Volume charts re-fetch independently of the Performance Overview period selector; Weekly shows ~12 weeks, Monthly ~12 months, Yearly ~5 years, each correctly bucketed (verify the current month/week/year's real data lands in the last bucket, not one bucket early) |
| 13.13 ✅ | Top Products table              | Compare ranking against Reports → Sales/Finance for the same period               | Ranked by **gross profit** (not revenue or margin %); each row shows qty sold, revenue, and gross profit; a lower-revenue but higher-margin product can outrank a higher-revenue one |
| 13.14 ✅ | Slow Movers table                | A product with stock and zero sales in the last 30 days                           | Appears in the "Slow Movers" table with lifetime qty sold, qty in stock, and days since its last sale; a product that sold even once in the last 30 days does NOT appear |
| 13.14a ✅ | Slow Movers — never sold        | A product with stock that has never had any dispense activity                     | Shows a red "Never sold" badge instead of a day count, and ranks above every other slow mover                |
...
| 13.17 ✅ | Location filter                 | Pick a specific location from the new dropdown                                    | Every KPI, chart, and list on the page (KPI row, Sales Overview, period-scoped section, Fast/Slow Movers) re-scopes to that location only |
...
| 13.19 ✅ | Last updated + refresh          | Note the "Updated HH:MM:SS" timestamp, click the refresh icon                     | Timestamp updates to the current time; the KPI section, Sales Overview, and the period-scoped section all re-fetch |
| 13.20 ✅ | Monthly Sales trend badge       | Compare the badge against this-month-to-date vs the full previous calendar month  | Up/down arrow and percentage match the computed change                                                        |
...
| 20.2 ✅ | Open via header button           | Click/tap the search icon in the header, at any screen width                        | Same palette opens; icon-only on mobile widths, "Search… Ctrl K" on desktop                                     |
| 20.3 ✅ | Close                            | Press Escape (desktop), tap the × button (mobile), or tap/click the dark backdrop    | Palette closes, search term clears                                                                              |
| 20.13 ✅ | Mobile layout                    | Open the palette on a phone-width viewport, focus the input                         | Modal sits near the top (not pushed off-screen by the on-screen keyboard); input text stays 16px+ (no iOS auto-zoom-in on focus); results list scrolls independently within the modal |
| 20.14 ✅ | Mobile keyboard behavior         | Type a search term on a phone                                                       | No auto-capitalization of the first letter, no autocorrect/spellcheck squiggles on product codes or DSP numbers  |
